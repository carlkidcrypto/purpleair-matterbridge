#!/usr/bin/env python3
"""Audit purpleair-matterbridge documentation for common issues and typos.

This script scans documentation files (*.rst, *.md) and source files under
``src/`` for known issues:
- Class name casing typos (e.g. PurpleairPlatform vs PurpleAirPlatform).
...
CHECKS: list[tuple[str, re.Pattern[str], str | None]] = [
    ...
    (
        "Incorrect class casing 'PurpleairPlatform' (should be 'PurpleAirPlatform')",
        re.compile(r"\bPurpleairPlatform\b"),
        "PurpleAirPlatform",
    ),
    (
        "Incorrect class casing 'PurpleairClient' (should be 'PurpleAirClient')",
        re.compile(r"\bPurpleairClient\b"),
        "PurpleAirClient",
    ),
    (
        "Incorrect casing 'Purpleair' in documentation (should be 'PurpleAir')",
        re.compile(r"\bPurpleair\b"),
        "PurpleAir",
    ),
]
...
def collect_files() -> list[Path]:
    """Return all documentation and source files to audit."""
    patterns = [
        "**/*.rst",
        "**/*.md",
        "src/**/*.ts",
...
def main() -> int:
    ...
    parser = argparse.ArgumentParser(
        description="Audit purpleair-matterbridge documentation."
    )
    parser.add_argument(
        "--fix",
        action="store_true",
