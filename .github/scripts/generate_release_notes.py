#!/usr/bin/env python3
"""Generate and update GitHub release notes for purpleair-matterbridge.

This script automates the generation of high-signal, human-readable release
notes for tagged purpleair-matterbridge releases, following the specification in
``.github/workflows/auto_release_notes.md``.

It deterministically:
1. Identifies release context (tag, prerelease status, npm version).
...
4. Generates a formatted release note matching the purpleair-matterbridge template.
5. Optionally patches the GitHub release via the GitHub CLI (gh).

Usage:
    # Process a single release tag (dry-run):
    python3 .github/scripts/generate_release_notes.py --tag v1.0.9

    # Process and publish a single release tag:
    python3 .github/scripts/generate_release_notes.py --tag v1.0.9 --publish
...
DEFAULT_REPO = "carlkidcrypto/purpleair-matterbridge"
DEFAULT_PACKAGE = "purpleair-matterbridge"
...
def categorize_item(title: str, files: list[str]) -> str:
    ...
    # 6. Runtime / Plugin / Bug Fixes / Features
    touches_runtime = any(
        f.startswith("src/")
        or f.endswith((".ts", ".js", ".config.json", ".schema.json"))
        for f in files
    )
    is_bug = any(
        w in title_lower
...
def synthesize_summary(
    ...
    if categories["Runtime / Plugin"]:
        highlights.append("core Matterbridge plugin and Matter endpoint improvements")
    if categories["Containers / Packaging"]:
        highlights.append("container images and packaging infrastructure")
    if categories["Tests"]:
    ...
    summary = f"This {release_type} focuses on {hl_str} across the purpleair-matterbridge package."
...
def build_release_notes(
    ...
    """Build formatted release notes markdown adhering to the purpleair-matterbridge template."""
    tag = release["tag_name"]
    is_prerelease = release.get("prerelease", False)
    npm_version = tag[1:] if tag.startswith("v") else tag
    has_valid_npm = bool(re.match(r"^\d+(\.\d+)*", npm_version))
...
def main() -> int:
    ...
    parser = argparse.ArgumentParser(
        description="Generate and update GitHub release notes for purpleair-matterbridge."
    )
